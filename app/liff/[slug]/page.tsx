    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ animation: 'beeFloat 1.8s ease-in-out infinite' }}>
          {tenant?.logoUrl
            ? <img src={tenant.logoUrl} alt={brandName} style={{ width: 100, height: 100, objectFit: 'contain' }} />
            : <BeeLogoSVG size={100} />
          }
        </div>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 36, fontWeight: 900, color: '#1F1F1F', letterSpacing: '-0.02em', lineHeight: 1 }}>
            {brandName}
          </p>
        ...
        <style>{`
          @keyframes beeFloat { 0%,100%{transform:translateY(0)rotate(-2deg)}50%{transform:translateY(-8px)rotate(2deg)} }
          @keyframes dotBounce { 0%,80%,100%{transform:scale(1);opacity:.3}40%{transform:scale(1.4);opacity:1} }
        `}</style>
      </div>
