      ...
      <div style={{ padding: '16px 16px 0', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <div style={{
            ...
            <input
              type="text"
              placeholder="搜尋目的地，例如：日本"
              value={query}
              onChange={e => setQuery(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleSearch()}
              style={{
                flex: 1, border: 'none', outline: 'none', background: 'none',
                fontSize: 16, color: '#1a1a1a', padding: '13px 0',
              }}
            />
            {query && (
        ...
        <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {DAY_CHIPS.map(d => (
              <button key={d} onClick={() => setSelDays(selDays === d ? null : d)}
                style={{
                  flexShrink: 0, padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer',
                  background: selDays === d ? C.primary : '#fff',
                  color: selDays === d ? '#fff' : '#4b5563',
                  border: selDays === d ? `1.5px solid ${C.primary}` : '1.5px solid rgba(0,0,0,0.1)',
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {DATA_CHIPS.map(d => (
              <button key={d} onClick={() => setSelData(selData === d ? null : d)}
                style={{
                  flexShrink: 0, padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer',
                  background: selData === d ? C.primary : '#fff',
                  color: selData === d ? '#fff' : '#4b5563',
                  border: selData === d ? `1.5px solid ${C.primary}` : '1.5px solid rgba(0,0,0,0.1)',
                  transition: 'all 0.15s',
                }}>{d}</button>
            ))}
          </div>
        ...
        {filtered.length > 0 && (
          <div style={{
            position: 'absolute', top: 'calc(100% - 0px)', left: 16, right: 16, zIndex: 30,
            background: '#fff', borderRadius: 14, border: '1px solid rgba(0,0,0,0.08)',
            boxShadow: '0 8px 24px rgba(0,0,0,0.1)', overflow: 'hidden',
            marginTop: 4, animation: 'dropIn 0.15s ease',
          }}>
            {filtered.map((c, i) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {hot.map((c, i) => (
              <button key={c.countryCode} onClick={() => onSelectCountry(c.countryCode)}
                style={{
                  background: getGradient(c.countryCode), borderRadius: 18, border: 'none', cursor: 'pointer',
                  padding: '18px 16px', textAlign: 'left', minHeight: 110,
                ...
                {i < 2 && (
                  <div style={{ position: 'absolute', top: 10, right: 10, background: '#ef4444', color: '#fff', fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', padding: '2px 6px', borderRadius: 6 }}>HOT</div>
                )}
